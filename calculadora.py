...
def press(val):
    equation.set(equation.get() + str(val))

def equalpress():
    try:
        total = str(eval(equation.get())) # eval, trata de resolver un string
        equation.set(total)
    except:
        equation.set("error")
...
expression_entry = Entry(root, textvariable=equation, width=64)
